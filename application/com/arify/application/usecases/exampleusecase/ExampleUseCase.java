...
public class ExampleUseCase implements ExamplePort {
    ...
    public ExampleUseCase(IFakeApiInfrastructure fakeApi, CacheLibraryService cacheLibrary) {
    ...
    public EasyResult<CreateExampleAdapter> getDataAsync(
        ...
        CompletableFuture<Optional<FakeApiEntity>> result1Task = fakeApi.getUserAsync(ThreadLocalRandom.current().nextInt(1, 15), token);
        CompletableFuture<Optional<FakeApiEntity>> result2Task = fakeApi.getTitleAsync(ThreadLocalRandom.current().nextInt(1, 15), token);
